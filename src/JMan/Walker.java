...
public class Walker extends Piece {
	...
	/**
	 * Constructor: a new Walker at position (x, y) on Map m with color c.
	...
	/**
	 * Walker should move based on what button is pushed. Random number and if
	 * number is more than 1, the walker will move
	 */
	public void act() {
		if (Math.random() * 2 > 1)
	...
	/**
	 * Move Walker one step based on the value of i: 0 = up, 1 = down, 2 = left,
	 * 3 = right.
	 */
	 ...
	 * 
	 * @param posX
	 *            is x position that walker want to move.
	 * @param posY
	 *            is y position that walker want to move.
	...
	public void draw(Graphics g, int x, int y, int width, int height) {
		g.setColor(getColor());
		g.fillOval(x + 1, y, width - 2, height - 2);
	}
}