...
public class Map {
	...
	public Map(int h, int w, int bl, int wa, int pi) {
		...
		bUp.addActionListener((evt) -> { jMan.step(0, -1); this.act();});
		bDown.addActionListener((evt) -> { jMan.step(0, 1); this.act();});
		bLeft.addActionListener((evt) -> { jMan.step(-1, 0); this.act();});
		bRight.addActionListener((evt) -> { jMan.step(1, 0); this.act();});
		bnewGame.addActionListener((evt) -> {
	 ...
	 * map must have enough empty spaces for all of the
	 */
	private void putOnMap(int bl, int wa, int pi) {
		...
		while (k < bl) {
			int xx = Piece.random(0, width - 1);
			int yy = Piece.random(0, height - 1);
			if (isEmpty(xx, yy)) {
	...
	public void putNew(int typ, int x, int y) {
		if (isEmpty(x, y)) {
			if (typ == Piece.JMAN) {
				jMan = new JMan(x, y, Piece.randomCl(), this);
				grid[x][y] = jMan;
			} else if (typ == Piece.BLOCK)
				grid[x][y] = new Block(x, y, this);
			else if (typ == Piece.PILLAR)
				grid[x][y] = new Pillar(x, y, Piece.randomCl(), this);
			else if (typ == Piece.WALKER)
				grid[x][y] = new Walker(x, y, Piece.randomCl(), this);
		}
	}
	...
	private class JManPanel extends JPanel {
		...
		/* Constructor: an instance with map */
		public JManPanel() {
		...
		public void paint(Graphics g) {
			...
			for (int i = 0; i < width; i = i + 1) {
				for (int j = 0; j < height; j = j + 1) {
					...
					if (piece != null)
						piece.draw(g, h, v, tileWidth, tileHeight);
				}
			} // end of inner class JManPan
		}
	}
}